...
class JobEstimate(models.Model):
    ...
    def _calculate_total(self):
        ...
        for order in self:
            for line in order.material_estimation_ids:
                ...
                order.total_material_estimate_m += material_price
                order.total_material_estimate_l += material_price
                order.total_material_estimate_o += material_price
            ...
            for line in order.labour_estimation_ids:
                ...
                order.total_labour_estimate_m += labour_price
                order.total_labour_estimate_l += labour_price
                order.total_labour_estimate_o += labour_price
            ...
            for line in order.overhead_estimation_ids:
                ...
                order.total_overhead_estimate_m += overhead_price
                order.total_overhead_estimate_l += overhead_price
                order.total_overhead_estimate_o += overhead_price
                total_job_cost += overhead_price
        
        order.total_job_estimate_m = total_job_cost
        order.total_job_estimate_l = total_job_cost
        order.total_job_estimate_o = total_job_cost
    ...
    # for Material Tab Total Cost
    total_material_estimate_m = fields.Float(compute='_calculate_total',string='Total Material Estimate',default=0.0,readonly=True)
    total_labour_estimate_m = fields.Float(compute='_calculate_total',string='Total Labour Estimate',default=0.0,readonly=True)
    total_overhead_estimate_m = fields.Float(compute='_calculate_total',string='Total Overhead Estimate',default=0.0,readonly=True)
    total_job_estimate_m = fields.Float(compute='_calculate_total',string='Total Job Estimate',default=0.0,readonly=True)
    
    # for Labour Tab Total Cost
    total_material_estimate_l = fields.Float(compute='_calculate_total',string='Total Material Estimate',default=0.0,readonly=True)
    total_labour_estimate_l = fields.Float(compute='_calculate_total',string='Total Labour Estimate',default=0.0,readonly=True)
    total_overhead_estimate_l = fields.Float(compute='_calculate_total',string='Total Overhead Estimate',default=0.0,readonly=True)
    total_job_estimate_l = fields.Float(compute='_calculate_total',string='Total Job Estimate',default=0.0,readonly=True)
    
    # for Overhead Tab Total Cost
    total_material_estimate_o = fields.Float(compute='_calculate_total',string='Total Material Estimate',default=0.0,readonly=True)
    total_labour_estimate_o = fields.Float(compute='_calculate_total',string='Total Labour Estimate',default=0.0,readonly=True)
    total_overhead_estimate_o = fields.Float(compute='_calculate_total',string='Total Overhead Estimate',default=0.0,readonly=True)
    total_job_estimate_o = fields.Float(compute='_calculate_total',string='Total Job Estimate',default=0.0,readonly=True)
...
class MaterialEstimate(models.Model):
    ...
    type = fields.Selection([('material','Material')],string='Type')
...
class LabourEstimate(models.Model):
    ...
    type = fields.Selection([('labour','Labour')],string='Type')
    product_id = fields.Many2one('product.product','Product',required=True)
    description = fields.Text('Description')
    quantity = fields.Float('Quantity',default=0.0)
    uom_id = fields.Many2one('product.uom','Unit Of Measure')
...
class OverheadEstimate(models.Model):
    ...
    type = fields.Selection([('overhead','Overhead')],string='Type')
    product_id = fields.Many2one('product.product','Product',required=True)
    description = fields.Text('Description')
    quantity = fields.Float('Quantity',default=0.0)
    uom_id = fields.Many2one('product.uom','Unit Of Measure')
