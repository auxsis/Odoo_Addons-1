...
class JobEstimate(models.Model):
    ...
    def create_quotation(self):
        ...
        for job_labour_line in self.labour_estimation_ids:
            uom = False
            if job_labour_line.uom_id:
                uom = job_labour_line.uom_id.id
            else:
                uom = job_labour_line.product_id.product_tmpl_id.uom_id.id
            if uom == False:
                raise UserError(_('Please Select Unit of Measure in labour estimate for '+job_labour_line.product_id.name+ ' product.'))
    ...
    state = fields.Selection([('draft','Draft'),('confirmed','Confirmed'),('approved','Approved'),('done','Quotation Created'),('cancel','Cancel')],'Status',copy=False,default="draft")#,('send','Estimate Sent')
