...
class CrafitoEcommerceShop(website_sale):
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        # For Price slider
        product_slider_ids = []
        asc_product_slider_ids = product_obj.search(cr, uid, [], limit=1, order='list_price', context=context)
        asc_products_slider = product_obj.browse(cr, uid, asc_product_slider_ids, context=context)
        desc_product_slider_ids = product_obj.search(cr, uid, [], limit=1, order='list_price desc', context=context)
        desc_products_slider = product_obj.browse(cr, uid, desc_product_slider_ids, context=context)
            ...
            if request.session.get('min1') or request.session.get('max1'):
                if request.session.get('min1'):
                    if request.session['min1'] != None:
                        slid_product_ids=product_obj.search(cr, uid, [('list_price', '>=', request.session.get('min1')), ('list_price', '<=', request.session.get('max1'))], context=context)
                        slid_products = product_obj.browse(cr, uid, slid_product_ids, context=context)
