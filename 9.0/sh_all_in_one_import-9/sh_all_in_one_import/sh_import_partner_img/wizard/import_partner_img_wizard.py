...
class import_partner_img_wizard(models.TransientModel):
    ...
    def show_success_msg(self,counter,skipped_line_no):
        ...
        action = self.env.ref('sh_all_in_one_import.sh_import_partner_img_action').read()[0]
                    ...
                    for row in myreader:
                        try:
                            ...
                            if row[0] not in (None,"") and row[1].strip() not in (None,""):
                                ...
                                search_partner = partner_obj.search([(field_nm,'=',row[0] )], limit = 1)                             
                                if search_partner:
                                ...
                                skipped_line_no[str(counter)] = " - Customer or URL/Path field is empty. "   
                                counter = counter + 1 
                                continue                                                                     
                    ...
                    for row in range(sheet.nrows):
                        try:
                            ...
                            if sheet.cell(row,0).value not in (None,"") and sheet.cell(row,1).value.strip() not in (None,""):
                                ...
                                search_partner = partner_obj.search([(field_nm,'=',search_str )], limit = 1)                             
                                if search_partner:
                                    search_partner.write(vals)
                                else:
                                    skipped_line_no[str(counter)] = " - Customer not found. "   
