...
class import_so_wizard(models.TransientModel):
    ...
    def show_success_msg(self,counter, confirm_rec, skipped_line_no):
        ...
        action = self.env.ref('sh_all_in_one_import.sh_import_so_action').read()[0]
                ...
                try:
                    ...
                    for row in myreader:
                        try:
                            ...
                            if row[0] not in (None,"") and row[4] not in (None,""): 
                                ...
                                if row[0] != running_so:
                                    ...
                                    if row[1] not in (None,""):
                                        ...
                                        elif self.is_create_customer:
                                            partner = partner_obj.create({'company_type' : 'person',
                                                                                 'name'         : row[1],
                                                                                 'customer'     : True,
                                                                                 })
                                            if not partner:
                                                skipped_line_no[str(counter)]= " - Customer not created. " 
                                                counter = counter + 1 
                                                continue
                                            else:
                                                so_vals.update({'partner_id' : partner.id})
                                        else:
                                            skipped_line_no[str(counter)]= " - Customer not found. " 
                                            counter = counter + 1 
                                            continue   
                                    else:
                                        skipped_line_no[str(counter)]= " - Customer field is empty. " 
                                        counter = counter + 1 
                                        continue     
                                    ...
                                    if row[3] not in (None,""):
                                        search_user = self.env['res.users'].search([('name','=',row[3] )], limit = 1)
                                        if search_user:
                                            so_vals.update({'user_id' : search_user.id})
                                        else:
                                ...
                                if created_so:  
                                    ...
                                    search_product = self.env['product.product'].search([(field_nm,'=',row[4])], limit = 1)
                                    if search_product:
                                        ...
                                        if row[6] != '':
                                            vals.update({'product_uom_qty' : row[6] })
                                        else:
                                        ...
                                        if row[7] in (None,"") and search_product.uom_id:
                                            ...
                                            if search_uom:
                                                ...
                                                if created_so.id in created_so_list_for_confirm:
                                                    created_so_list_for_confirm.remove(created_so.id)
                                                continue
                                        ...
                                        if row[9].strip() in (None,"") and search_product.taxes_id:
                                            vals.update({'tax_id' : [(6, 0, search_product.taxes_id.ids)]})
                                        else:
                                            taxes_list = []
                                        ...
                                        vals.update({'order_id' : created_so.id})
                                        sol_obj.create(vals)
                                        counter = counter + 1
                                ...
                                skipped_line_no[str(counter)] = " - Sales Order or Product field is empty. "  
                                counter = counter + 1      
                            ...
                            counter = counter + 1 
                            continue          
                    if created_so_list_for_confirm and self.is_confirm_sale == True:
                        sale_orders = sale_order_obj.search([('id','in',created_so_list_for_confirm)])
                        if sale_orders:
                            ...
                            if sheet.cell(row,0).value not in (None,"") and sheet.cell(row,4).value not in (None,""): 
                                ...
                                if sheet.cell(row,0).value != running_so:
                                    ...
                                    if sheet.cell(row,3).value not in (None,""):
                                        search_user = self.env['res.users'].search([('name','=', sheet.cell(row,3).value )], limit = 1)
                                        if search_user:
                                            so_vals.update({'user_id' : search_user.id})
                                        else:
                                            skipped_line_no[str(counter)]= " - Salesperson not found. " 
                                    ...
                                    if self.order_no_type == 'as_per_sheet':
                                        so_vals.update({"name" : sheet.cell(row,0).value })                           
                                    created_so = sale_order_obj.create(so_vals)     
                                    created_so_list_for_confirm.append(created_so.id)         
                                    created_so_list.append(created_so.id)                    
                                ...
                                if created_so:  
                                    ...
                                    search_product = self.env['product.product'].search([(field_nm,'=',sheet.cell(row,4).value )], limit = 1)
                                    if search_product:
                                        vals.update({'product_id' : search_product.id})
                                         
                                        if sheet.cell(row,5).value != '':
                                            vals.update({'name' : sheet.cell(row,5).value })
                                             
                                        if sheet.cell(row,6).value != '':
                                            vals.update({'product_uom_qty' : sheet.cell(row,6).value })
                                        else:
                                            vals.update({'product_uom_qty' : 1 })
                                        ...
                                        if sheet.cell(row,8).value in (None,""):
                                            ...
                                            vals.update({'price_unit' : sheet.cell(row,8).value })
